...
#define TEST5_SIZE 2000

namespace test5 {
int test5(int argc, char *argv[])
{
    int rank, nprocs, i, A[TEST5_SIZE], B[TEST5_SIZE];
    MPI_Comm CommDeuce;
    MPI_Win win;
    int errs = 0;
    ...
    if (rank < 2) {
        if (rank == 0) {
            for (i = 0; i < TEST5_SIZE; i++)
                B[i] = 500 + i;
            MPI_Win_create(B, TEST5_SIZE * sizeof(int), sizeof(int), MPI_INFO_NULL, CommDeuce, &win);
            MPI_Win_fence(0, win);
            for (i = 0; i < TEST5_SIZE; i++) {
                A[i] = i + 100;
        ...
        if (rank == 1) {
            for (i = 0; i < TEST5_SIZE; i++)
                A[i] = 1000 + i;
            MPI_Win_create(A, TEST5_SIZE * sizeof(int), sizeof(int), MPI_INFO_NULL, CommDeuce, &win);
            MPI_Win_fence(0, win);
            for (i = 0; i < TEST5_SIZE; i++) {
                B[i] = i + 200;
